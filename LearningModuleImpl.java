...
public class LearningModuleImpl implements LearningModule {
    ...
    private int actionPosition = 100; // action is a___ in stateCode 2, 
    ...
    @Override
    public Boolean updateTrafficLights( RoadMap r, Car c, int timeRan) {
      
    	List<Boolean> switches = new ArrayList<Boolean>();
       	
        Action aa=getAction(r,c);
        boolean a =		aa.action();
        
        c.setStrategy(aa.actionInt());
    ...
    public List<Boolean> updateTrafficLightsRandomly(
            RoadMap mapWithCars, List<Car> Game_cars) {
    ...
    @Override
    public void learn(Integer pastState, Boolean switche,Integer reward1, Integer newState,Car c) {
        updateAlpha();

        //use old hashcode and current state
        //for (int i = 0; i < Game_cars.size(); i++) {
            //for each traffic light we get the state code, action, 
            //reward and old qvalue
            int state = pastState;
            int nextState = newState;
            Action action = new ActionImpl(switche);
            int reward = reward1;
   ...
   /* public int reward(int stateCode) {
    ...
    public Action getAction(RoadMap r, Car c) {
        ActionImpl a;
        ...
        for (int i = 0; i < numActions; i++) {
            a = actions[i];
            Float q = qValues.get(r.stateCode(c,r) + actionPosition*i);
 ...
 //System.err.println(highestAction.actionInt());
