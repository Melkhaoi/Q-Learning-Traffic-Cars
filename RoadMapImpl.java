...
public class RoadMapImpl implements RoadMap {
	...
	private final Coords[] defaultEntrances = { new Coords(19, 0), new Coords(39, 0), new Coords(59, 19),
			new Coords(59, 39), new Coords(41, 59), new Coords(21, 59), new Coords(0, 41), new Coords(0, 21) };

	private final int[] id = { 0, 1, 2, 3, 4, 5, 6, 7 };
		...
		for (int i = 0; i < gridSize; i++) {
			for (int j = 0; j < gridSize; j++) {
				...
				for (Coords k : roadEntrances) {
					if ((0 < i && i < gridSize - 1 && i == k.getX()) || (0 < j && j < gridSize - 1 && j == k.getY())) {
						grid[j][i] = roadChar;
					}
				}
			}
	...
	public void print(List<Car> cars, List<TrafficLight> trafficLights) {
		...
		for (Car car : cars) {
			...
			newGrid[y][x] = dx == 0 ? dy < 0 ? '^' : 'v' : dy == 0 ? dx < 0 ? '<' : '>' : '6';
		}
	...
	/*
	 * 1st - Num cars with velocity 0 on road 1 2nd - Num cars with velocity 0 on
	 * road 2 3rd - light setting (ie 0-green, 1 red for one of the roads) //we
	 * don't want to include amber as a light setting because //we do not learn when
	 * the light is amber
	 */
	
	public int stateCode(Car c,RoadMap m) {
		//if(c.leDroit(cars)==null)
		//	return 2;
		if(c.droit(m))
		return c.getStrategy()+10;
		else
        return c.getStrategy();
	
	}
	/*   public int stateCode(Car car,TrafficLight t) {
	        int hash = 0;

	        int lightSetting;
	        if (car.getStrategy()==1) {
	            lightSetting = 1;
	        } else {
	        	lightSetting = 0;
	        }
	...
	public TrafficLight getClosestTrafficLight(Car car, List<TrafficLight> trafficLights) {
		...
		while (coords.getX() >= 0 && coords.getY() >= 0 && coords.getX() < 60 && coords.getY() < 60) {
	...
	public Velocity getStartingVelocity(Coords roadEntrance) {
		...
		return new Velocity(y == 0 || y == gridSize - 1 ? 0 : x == 0 ? 1 : -1,
	...
	public boolean nextNonCarSquareIsTrafficLight(Coords start, Velocity direction, TrafficLight trafficLight) {
		...
		while (current.getX() < gridSize && current.getX() >= 0 && current.getY() < gridSize && current.getY() >= 0
				&& carAt(current) && !trafficLightCoords.equals(current)) {
			current.setX(current.getX() + direction.getXSpeed());
			current.setY(current.getY() + direction.getYSpeed());
		}
	...
	public boolean carAt(Coords coords) {
		...
		return 0 <= coords.getX() && coords.getX() < gridSize && 0 <= coords.getY() && coords.getY() < gridSize
	...
	public Coords trafficLightCoords(Velocity direction, TrafficLight trafficLight) {
		...
		if (direction.getXSpeed() == 0) {
			...
			if (direction.getXSpeed() == 1) {
				trafficLightCoords = trafficLight.getCoords().down().left();
